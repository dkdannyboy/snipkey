...
- **大文字小文字に合わせる。** `;Sig` なら "Best regards"、`;SIG` なら "BEST REGARDS"。
- **Backspace で取り消し。** 展開直後に Backspace を押すと、入力した略語に戻ります。
- **アプリごとの除外。** ターミナルや IDE など、好きなアプリで展開をオフに — メニューバーから直接。
- **Windows 版も（プレビュー）。** [SnipKey for Windows](windows/README.md) は同じライブラリ
  ファイルを読むので、Mac と PC で 1 つのライブラリを共有できます。
...
| `%date:+1d:yyyy-MM-dd%` | 日付の計算: `+`/`-` と数字、単位 `s` `m` `h` `d` `w` `M` `y`（分は `m`、月は `M`） |
| `%Y` `%m` `%d` `%B` `%A` … `%@+1D` | TextExpander の日付コード — 読み込んだスニペットがそのまま動きます |
