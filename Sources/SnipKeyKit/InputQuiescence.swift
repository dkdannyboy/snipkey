...
public enum ExpansionDecision: Equatable {
    /// 가드를 무장한 뒤로 진짜 키가 하나도 없었다. 지우고 붙여넣어도 된다.
    case proceed
    /// 무장 이후에 사용자가 대상 앱에 키를 넣었다. 확장을 통째로 버린다.
    /// `keyArrivedAt`은 그 마지막 키의 시각(로그용).
    case abort(keyArrivedAt: TimeInterval)
}

/// "지금 지워도 되는가"를 판정한다.
///
/// 묻는 질문은 하나다: **무장한 시점 이후에 대상 앱으로 들어간 진짜 키가 있는가.**
/// 있으면 버린다.
///
/// 왜 이 장치가 필요한가 — 이벤트 탭이 `.listenOnly`라 사용자의 키는 대상 앱에 '먼저'
/// 도착한다. 확장은 비동기이고 느리다. 'sig ' 가 매치된 뒤에도 사용자가 'next'를 계속
/// 치면, 그 글자들이 이미 앱에 들어간 상태에서 백스페이스 4번이 나간다 — 지워지는 것은
/// 'sig '가 아니라 'next'다. 확장이 안 되는 것보다 엉뚱한 자리에서 확장되는 것이 나쁘다.
/// 타이핑을 이어간 사용자는 확장을 못 볼 뿐이지만(다시 치고 잠깐 멈추면 된다), 그
/// 상태로 백스페이스를 맞은 사용자는 자기가 쓴 글자를 조용히, 되돌릴 수 없이 잃는다.
///
/// 왜 '정숙 시간'이 아니라 '무장 이후 입력 여부'인가 — 한때 "마지막 키로부터 N ms가
...
/// 돌아온 뒤에 친 키는 언제나 무장 이후다.
public struct InputQuiescenceGuard: Equatable {

    /// 이 시각 이후에 도착한 진짜 키가 있으면 확장을 버린다.
    ...
    ///
    /// - Parameter lastRealKeyAt: 마지막 '진짜' 사용자 keyDown의 **이벤트 시각**.
    ///   우리가 그 이벤트를 처리한 시각이 아니다. nil이면 아직 아무 키도 관측하지 않았다.
    public func decide(lastRealKeyAt: TimeInterval?) -> ExpansionDecision {
        guard let lastRealKeyAt, lastRealKeyAt > armedAt else { return .proceed }
        return .abort(keyArrivedAt: lastRealKeyAt)
    }
}

/// 마지막 '진짜' 사용자 키의 이벤트 시각만 들고 있는 시계.
///
/// 이벤트 탭 콜백(메인 런루프)에서 쓰고 인젝터 큐에서 읽으므로 잠금이 필요하다.
public final class KeystrokeClock: @unchecked Sendable {
    ...
    /// 아니다 — 타임베이스(numer/denom)를 곱하면 값이 40배 가까이 부풀어서 모든 키가
    ...
    public init(now: @escaping () -> TimeInterval = KeystrokeClock.monotonicNow) {
        self.clock = now
    }

    /// 진짜 키가 하나 도착했다. 반드시 그 키의 **이벤트 시각**을 넘겨야 한다 —
    /// 우리가 콜백에서 관측한 시각을 넘기면 패널 종료 키와의 경합이 되살아난다.
    ///
    ...
    public var lastKeyAt: TimeInterval? {
        lock.lock()
        defer { lock.unlock() }
        return last
    }

    /// 지금을 기준으로 가드를 무장한다. 이 순간 이후에 도착하는 키가 확장을 취소시킨다.
    ...
    public func decide(_ quiescence: InputQuiescenceGuard) -> ExpansionDecision {
        quiescence.decide(lastRealKeyAt: lastKeyAt)
    }
}
