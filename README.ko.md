...
- **대소문자 따라가기.** `;Sig`로 치면 "Best regards", `;SIG`로 치면 "BEST REGARDS".
- **백스페이스로 되돌리기.** 확장 직후 백스페이스를 누르면 입력했던 약어로 돌아갑니다.
- **앱별 제외.** 터미널·IDE 등 원하는 앱에서는 확장을 끕니다 — 메뉴 막대에서 바로.
- **Windows도 (미리보기).** [SnipKey for Windows](windows/README.md)는 같은 라이브러리
  파일을 읽으므로, Mac과 PC가 라이브러리 하나를 함께 쓸 수 있습니다.
...
| `%date:+1d:yyyy-MM-dd%` | 날짜 계산: `+`/`-` 숫자와 단위 `s` `m` `h` `d` `w` `M` `y` (분은 `m`, 월은 `M`) |
| `%Y` `%m` `%d` `%B` `%A` … `%@+1D` | TextExpander 날짜 코드 — 가져온 스니펫이 그대로 동작합니다 |
